...
public sealed class AutomatronScript : Script
{
    ...
    {
        return $"dotnet build /nodeReuse:false /p:UseSharedCompilation=false -nologo -clp:NoSummary --verbosity quiet && dotnet run -- run --stage {stage} --job {job} --step {step} {pipeline}";
    }
}